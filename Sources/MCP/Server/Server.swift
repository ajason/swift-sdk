...
public actor Server {
    ...
    task = Task {
      do {
        ...
        for try await data in stream {
          ...
          await logger?.debug("Server transport.receive data", metadata: ["data": "\(data)"])
          ...
          do {
            ...
            if let batch = try? decoder.decode(Server.Batch.self, from: data) {
              await logger?.debug("Server received batch", metadata: [:])
              try await handleBatch(batch)
            } else if let request = try? decoder.decode(AnyRequest.self, from: data) {
              await logger?.debug("Server received request", metadata: [:])
              _ = try await handleRequest(request, sendResponse: true)
            } else if let message = try? decoder.decode(AnyMessage.self, from: data) {
              await logger?.debug("Server received message", metadata: [:])
              try await handleMessage(message)
            } else {
              await logger?.debug("Server received invalid message", metadata: [:])
  ...
  private func registerDefaultHandlers(
    ...
    withMethodHandler(Initialize.self) { [weak self] id, params in
      ...
      await logger?.debug("Server received initialize request", metadata: ["params": "\(params)"])
